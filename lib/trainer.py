...
class Trainer:
    ...
    def train(self, data_t, data_v, epochs=100, gpu=None):
        while self.epoch <= epochs:
            print(f"Epoch {self.epoch}/{epochs}")
            stats_t = self._run_epoch(data_t, gpu, train=True)  # train epoch
            self.history[0] += [stats_t]
            stats_v = self._run_epoch(data_v, gpu, train=False)  # validate epoch
    ...
    def evaluate(self, data_e, gpu=None):
        print('Evaluate')
        stats_e = self._run_epoch(data_e, gpu, train=False)  # evaluate
        self.history[-1] += [stats_e]

    def _run_epoch(self, data, gpu, train=True):
        metrics, total_loss = Metrics(), .0
        with torch.set_grad_enabled(train):  # enable or disable autograd
            self.net.train(train)
            with ProgressBar(data) as bar:

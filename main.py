...
if __name__ == '__main__':
    ...
    loaders = load_data(args.data, args.batch, in_hours, args.out_timesteps, args.frequency, args.workers)
    # adjacency matrix
    adj = load_adj(args.adj, args.nodes)
    # network
    net = models[args.model](len(in_hours), args.channels, args.frequency, args.out_timesteps, adj, args.use_te)
    ...
    # train and validate
    trainer.train(loaders[0], loaders[1], epochs=args.epochs, gpu=args.gpu)
    # load best checkpoint
    trainer.load_checkpoint(f'epoch={trainer.best_epoch}_loss={trainer.min_loss:.2f}.pkl')
    # evaluate
    trainer.evaluate(loaders[-1], gpu=args.gpu)
    # save history
